...
class QLearningAgent:
    ...
    def update_q_network(self, state, action, reward, next_state, next_action):
        ...
        # print(next_q_values)
        # Compute the target Q-value using SARSA update rule
        target = reward + self.discount_factor * next_q_values[next_action]
...
for i in range(ds.shape[0]):
    ...
    q_agent.update_q_network(
        flatten_state(state_data), action, reward, flatten_state(next_state), action
    )
